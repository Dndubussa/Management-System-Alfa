-- Supabase Migration Script - Check and Update Only Missing Elements
-- This script checks what exists and only adds what does not exist
...
create or replace function constraint_exists(con_name text)
returns boolean as $$
begin
    return exists (
        select 1 
        from information_schema.table_constraints 
        where constraint_name = con_name
    );
end;
$$ language plpgsql;

-- Function to check if a trigger exists
create or replace function trigger_exists(trigger_name text)
returns boolean as $$
begin
    return exists (
        select 1 
...
-- Create or update tables based on what exists
do $$
begin
    -- Patients table
    ...
    if not table_exists('medical_records') then
    ...
    -- Prescriptions table
    if not table_exists('prescriptions') then
    ...
    -- Lab orders table
    if not table_exists('lab_orders') then
    ...
    if not table_exists('service_prices') then
    ...
    -- Bill items table
    if not table_exists('bill_items') then
    ...
    -- Departments table
    if not table_exists('departments') then
    ...
    -- Referrals table
    if not table_exists('referrals') then
    ...
    if not table_exists('insurance_claims') then
    ...
    if not table_exists('surgery_requests') then
    ...
    -- OT slots table
    if not table_exists('ot_slots') then
    ...
    -- OT resources table
    if not table_exists('ot_resources') then
    ...
    -- OT checklists table
    if not table_exists('ot_checklists') then
    ...
    if not table_exists('surgery_progress') then
    ...
    -- OT reports table
    if not table_exists('ot_reports') then
